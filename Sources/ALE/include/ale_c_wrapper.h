...
extern "C" {
    ...
    void deleteInterface(const void *ale);
    void loadROM(const void *ale, const char *rom_file);
    int act(const void *ale, int action);
    void getLegalActionSet(const void *object, int *actions, unsigned long *action_count);
    ...
    void saveScreenPNG(const void *ale, const char *file_path);

    /// API for future needs.
    /*
